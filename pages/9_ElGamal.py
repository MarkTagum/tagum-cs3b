...
if generate_keypair:
    p = st.number_input("Prime Number (p)", min_value=11)  
    ...
    if st.button("Generate Keys"):
        ...
        st.write("**Private Key (Keep Secret):**", private_key)
        public_key_p, public_key_g, public_key_h = public_key
    ...
    private_key = st.text_input("Private Key (if available)", type="password")
...
if encryption_mode == "Encrypt":
    ...
    if message and (public_key_p and public_key_g and public_key_h):
        bob = ElGamal(public_key_p, public_key_g)
    ...
    if public_key_p and public_key_g and public_key_h:
        ...
        if c1 and c2:
            bob = ElGamal(public_key_p, public_key_g, int(private_key))
